...
public class hashpath {
    ...
    public static boolean ispath(ArrayList<info>graph[],int curr,int end,boolean ans[]){
        if(curr==end){
          return true;
        }
        ans[curr]=true;
        for(int i=0;i<graph[curr].size();i++){
            ...
            if(!ans[neighbour] && ispath(graph, neighbour, end, ans)){
                return true;
            }
        }
        return false;
    ...
    public static void main(String[] args) {
       ...
       boolean val=ispath(graph,1,6,ans);
